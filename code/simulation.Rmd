...
- 추가사항: Idle vehicle이 다 배차되고도 승객의 수요가 충족이 안되면 Relocating 중인 차량을 배차시킴
...
- Density of idle vehicles (every 30-min)
```{r}
InitialVehicle <- data.frame(VehicleID=1:700)
InitialVehicle$X <- runif(nrow(InitialVehicle), min=0, max=7000)
InitialVehicle$Y <- runif(nrow(InitialVehicle), min=0, max=6000)
...
```{r}
RelocationTableCalculator <- function (i=1,pmu=0.7,pcro=0.2,max_iter=100){
...
cons <- function(z) {
  ...
  con<-0
  zone_cons <- A_delta[[i]] - 0.1*(SD_Phi[[i]] + SD_Psi[[i]]) + x
...
InterReloTableGA[[1]] <- RelocationTableCalculator(i=1)
```
...
InterZonalRepositioner<-function(i=1){
  ...
  if(nrow(inter_relo_vehicle_tmp)!=0) InterReloVehicle[[i]] <<- inter_relo_vehicle_tmp
  
  # IdleVehicle
  IdleVehicle[[i]] <<- idle_vehicle_tmp
  IdleVehicle[[i+1]] <<- IdleVehicle[[i+1]] %>% filter(!VehicleID %in% inter_relo_vehicle_tmp$VehicleID)
}
```
...
nrow(IdleVehicle[[299]])+nrow(AssignVehicle[[299]])+nrow(OprtVehicle[[299]])
#nrow(IdleVehicle[[2]])+nrow(AssignVehicle[[2]])+nrow(OprtVehicle[[2]])+nrow(InterReloVehicle[[2]])
...
MakeVisDat<- function(type="inter-relo"){
  ...
  if (type=="inter-relo") {
    agg_InterReloVehicle <- do.call(rbind,sapply(1:100, function (k) InterReloVehicle[[k]], simplify = FALSE))
...
```{r}
Run_Simulation<-function(relocation="none"){
  IdleVehicle <- vector("list",1440)
  OprtVehicle <- vector("list",1440)
  AssignVehicle <- vector("list",1440)
...
CalculateAssignCost <- function(AssignVehicle){
  ...
  # 30분 단위로 표출
  #tmp2$time_30min <- trunc((as.numeric(tmp2$time)-1)/30)+1
...
qplot(AssignCost_default$time,AssignCost_default$total_assign_time,geom="line")+ylim(0,250)+xlim(0,24)

AssignCost_inter_relo<-CalculateAssignCost(AssignVehicle_inter_relo)
qplot(time,total_assign_time,data=AssignCost_inter_relo,geom="line")+ylim(0,250)+xlim(0,24)
```

#### Relocation cost

```{r}
CalculateInterRelocationCost <- function(){
