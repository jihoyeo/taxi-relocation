...
for (i in 1:1439){
  ...
  if((i-1)%%30==0) {
    InterReloTableGA[[(i-1)/30+1]] <- RelocationTableCalculator_intra((i-1)/30+1,max_iter=1, type="intra_s1")
    InterReloTable[[i]]<-rbind(InterReloTable[[i]], ConvertTable(InterReloTableGA[[(i-1)/30+1]]))
    PodInfoCalculator(P_n,PodCoord,i) # Calculate pod information (e.g, number, location)
  }
...
vis_dat_inter_relo <- MakeVisDat(type="inter-intra-relo")
